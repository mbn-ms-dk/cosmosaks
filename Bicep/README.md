...
az deployment sub create --name 'env-create' Name --location $location --template-file main.bicep --parameters @param.json
```
...
Using the following YAML template create a secretproviderclass.yml. Make sure to update your own values for {Tenant Id} and {Keyvault Name} placeholders. Ensure that the below values for {Resource Group Name} placeholder matches with values supplied in param.json.
...
spec:
  ...
  parameters:
    usePodIdentity: "true"               
    keyvaultName: "{Keyvault Name}"       # Replace resource group name. Key Vault name is generated by Bicep
    cloudName: ""                        
    objects:  |
      array:
        - |
        ...
        - |
          ...
          objectType: key
          objectVersion: ""
    tenantId: "{Tenant Id}"              # The tenant ID of your account, use 'homeTenantId' attribute value from  the 'az account show' command
```
